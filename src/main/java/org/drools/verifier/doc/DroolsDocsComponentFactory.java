...
public class DroolsDocsComponentFactory {
   ...
   // Font fontChinese = new Font(BaseFont.createFont("STSong-Light" ,"UniGB-UCS2-H", BaseFont.NOT_EMBEDDED), 12, Font.NORMAL);
    ...
    private static final Font   CHAPTER_TITLE      = FontFactory.getFont( "STSongStd-Light", "UniGB-UCS2-H", BaseFont.NOT_EMBEDDED,20, Font.BOLD);
    			
    private static final Font   PACKAGE_NAME    = FontFactory.getFont("STSongStd-Light", "UniGB-UCS2-H", BaseFont.NOT_EMBEDDED,10, Font.BOLD);
    
    private static final Font   RULE_PACKAGE_TITLE = FontFactory.getFont("STSongStd-Light", "UniGB-UCS2-H", BaseFont.NOT_EMBEDDED,24, Font.BOLD);
    
    private static final Font   CATEGORIES_TEXT    = FontFactory.getFont("STSongStd-Light", "UniGB-UCS2-H", BaseFont.NOT_EMBEDDED,12, Font.NORMAL);
	private static final Font   CATEGORIES_TEXT_EN    = FontFactory.getFont( FontFactory.TIMES, 12 );
	
    private static final Font   BODY_TEXT          = FontFactory.getFont("STSongStd-Light", "UniGB-UCS2-H", BaseFont.NOT_EMBEDDED,10, Font.NORMAL);
	private static final Font   BODY_TEXT_EN          = FontFactory.getFont( FontFactory.TIMES,10 );
	
    static final Font           HEADER_FOOTER_TEXT =FontFactory.getFont("STSongStd-Light", "UniGB-UCS2-H", BaseFont.NOT_EMBEDDED,8, Font.NORMAL);
	static final Font           HEADER_FOOTER_TEXT_EN = FontFactory.getFont( FontFactory.TIMES,   8 );
    private static final Font   CHAPTER_TITLE_EN      = FontFactory.getFont( FontFactory.TIMES,
            20,
	            Font.BOLD );
    /*
	...
	private static final Font   BODY_TEXT          = FontFactory.getFont( FontFactory.TIMES,
	            10 );
	static final Font           HEADER_FOOTER_TEXT = FontFactory.getFont( FontFactory.TIMES,   8 );*/
    ...
    public static Table newDescription(String description) throws DocumentException {
        ...
        /*Cell headerCell = newHeaderCell( "Description",
                                         CATEGORIES_TEXT );*/
        Cell headerCell = newHeaderCell( DroolsDocConstant.DESCRIPTION,
                CATEGORIES_TEXT );
...
/*        Cell headerCell = newHeaderCell( "Attributes",
                                         CATEGORIES_TEXT );*/
        Cell headerCell = newHeaderCell(DroolsDocConstant.ATTRIBUTES,
                CATEGORIES_TEXT );
        table.addCell( headerCell );
        ...
        table.addCell( newHeaderCell( INDENT + DroolsDocConstant.WHEN,
                                      BODY_TEXT ) );
        for ( String s : drl.getLhs() ) {
            table.addCell( newCell( INDENT + INDENT + s.trim() ) );
        }

        table.addCell( newHeaderCell( INDENT + DroolsDocConstant.THEN,
        		BODY_TEXT ) );
...
/*        document.add( new Paragraph( "Rule " + drlData.getName(),
        ...
        document.add( new Paragraph( DroolsDocConstant.RULE + drlData.getName(),
                CHAPTER_TITLE ) );
...
/*            document.add( new Paragraph( "Extends:",
            ...
            document.add( new Paragraph( DroolsDocConstant.EXTENDS,
            		BODY_TEXT) );
        ...
        /*       document.add( newTable( "Metadata",
                                drlData.getMetadata() ) );*/
        document.add( newTable( DroolsDocConstant.METDDATA,
        		drlData.getMetadata() ) );
    ...
    public static void createFirstPage(Document document,
     ...
     /*   Paragraph date = new Paragraph( "Documentation created: " + currentDate,
                                        BODY_TEXT );*/
        Paragraph date = new Paragraph( DroolsDocConstant.DOCUMENTATION_CREATED + currentDate,
                BODY_TEXT );
...
/*        document.add( newTable( "Metadata ",
        ...
        document.add( newTable(DroolsDocConstant.METDDATA,
                packageData.getMetadata() ) );
        document.add( newTable( DroolsDocConstant.GLOBALS,
                packageData.getGlobals() ) );
    ...
    public void onEndPage(PdfWriter writer,
        ...
        try {
            ...
            PdfPCell cell2 = new PdfPCell( new Phrase( currentDate,
                                                       DroolsDocsComponentFactory.HEADER_FOOTER_TEXT_EN ) );
